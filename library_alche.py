...
class category(db.Model):
    ...
    categorys = db.relationship('book', backref='info_category', lazy='dynamic')
...
def get_borrow(current_user):
    borrowed_books = borrow.query.filter_by(user_id=current_user.id, status=True)
    returned_books = borrow.query.filter_by(user_id=current_user.id, status=False, confirmation=False)
    all_books = borrowed_books.union(returned_books).all()
    ...
    result = [
        {
            'info_book': {
                ...
                'category': el.info_book_borrow.info_category.name,
            },
            'info_borrow': {
                'id': el.id,
...
def add_borrow(current_user):
    ...
    return jsonify(
        {'info_book':        
            ...
            'category': new_borrow.info_book_borrow.info_category.name
            },
        'info_borrow':
        ...
        'info_date':
            ...
            'return_date': new_borrow.return_date.strftime('%Y-%m-%d')if new_borrow.return_date else None
    }})
...
def update_return_confirm(current_user):
    ...
    return jsonify(
        {'info_borrow':
            ...
            'status': 'borrow' if borrow_entry.status else 'returned',
            'fine': f'Rp {borrow_entry.days_late * 1000}'        
            },
            'info_date':
...
def update_borrow_status():
    ...
    borrow_entry = borrow.query.filter_by(id=borrow_id, status=False).first()
    if not borrow_entry:
        return jsonify({
            'error': 'not found',
            'message': 'no borrowing entry found for the specified id'
    ...
    return jsonify(
        {'info_book':
            ...
            'category': borrow_entry.info_book_borrow.info_category.name
            },
        'info_borrow':
            {'id': borrow_entry.id,
             'user': borrow_entry.info_user.username,
            'confirm_by': 'admin' if borrow_entry else 'unconfirmed',
            'fine': f'Rp {borrow_entry.days_late * 1000}',
            'charge': f'Rp {(borrow_entry.end_date - borrow_entry.start_date).days * 1000}', 
            'status': 'borrow' if borrow_entry.status else 'returned',
            },
        'info_date':
